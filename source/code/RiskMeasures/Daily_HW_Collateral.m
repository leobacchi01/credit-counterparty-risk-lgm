function [collateral_Epsilon, collateral_Delta, expected_exposure_delta,expected_positive_exposure_delta,potential_future_exposure_delta,peak_pfe_delta, expected_exposure_epsilon,expected_positive_exposure_epsilon,potential_future_exposure_epsilon,peak_pfe_epsilon] = Daily_HW_Collateral(Portfolio, dates, discounts, simulations_num, alpha, aBach, sigmaBach)
...
EU_30_360=6;
ACT_365=3;
% Mean-reversion speed for Hull-White
a=aBach;
...
% Simulation grid excludes t = 0 (we use days 2..end)
...
% Preallocate 3D arrays to store MtM trajectories per counterparty
r = simulations_num; c = N;   
TLGM_Epsilon = zeros(r, c, nEpsilon);
TLGM_Delta = zeros(r, c, nDelta);
% Preallocate array for the simulated short-rate factor x(t)
xtsHW = zeros(simulations_num, N);

countDelta = 1; 
countEpsilon = 1; 
% 1) Simulate the Hull-White x(t) factor daily
for idx = 1:length(simulations_grid)
    ...
    % Store x(t) for all scenarios at this index
    xtsHW(:, idx) = x_t;
end

% 2) Loop for each swap
for j=1:length(maturity)
% Preallocate MtM matrices for this swap
MtMs_Epsilon=zeros(simulations_num, N);
MtMs_Delta=zeros(simulations_num, N);

    % Build the swap-specific daily timeline up to its maturity
    if maturity(j) >= 1
...
simulations_grid = simulations_grid(2:end);  % exclude t = 0
Nstep           = numel(simulations_grid);
% At the first simulation date, precompute the affine coefficients A, C
sim_date = simulations_grid(1);
[A, C] = affine_trick(sim_date, simulations_grid, aBach, sigmaBach, discounts_timegrid, dates, discounts);

% Loop through each day in this swap’s timeline
for idx = 1:length(simulations_grid)
    sim_date = simulations_grid(idx);
    x_t=xtsHW(:, idx)';
...
% 4) Compute collateralized MtM for Epsilon
for i=1:N-1
    if mod(i,7) == 0
        % Every 7th day (weekly interval), post collateral:
        ...
        collateral_Epsilon(:, i) = - MtM_coll_Epsilon(:, i); % post new collateral
        MtM_coll_Epsilon(:, i) = 0; % MtM reset after collateral
    elseif i==1
        % At first step, no prior collateral to adjust
        MtM_coll_Epsilon(:, i) = MtM_coll_Epsilon(:, i) ;
    else
        % On non‐collateral posting days, accrue previous collateral at stochastic discount
        stoch_disc = computeStochDiscHW(today,simulations_grid, i, ACT_365,a,sigmaBach,xtsHW,fwd_disc);
        MtM_coll_Epsilon(:, i) = MtM_coll_Epsilon(:, i) + collateral_Epsilon(:, i-1)./stoch_disc;
    end
end

% 5) Compute collateralized MtM for Delta
for i=1:N-1
    if mod(i,7) == 0
        stoch_disc = computeStochDiscHW(today,simulations_grid, i, ACT_365,a,sigmaBach,xtsHW,fwd_disc);
...
% Replace any NaN entries (due to zero‐probability) with 0
